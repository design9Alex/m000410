  ...
  <div class="form-group row" :class="{'len': configs.language}">
    ...
    <div class="col-sm-10 table-responsive">
      <table class="table table-sm table-bordered">
        ...
        <tbody>
        <tr v-for="(permissions, role) in this.value">
          <td>{{ roleTitle(role) }}</td>
          <td>
            <span v-for="(permission, index) in permissions">
...
export default {
  ...
  methods: {
    roleTitle: function (role_id) {
      for (let index in this.options.options) {
        if (this.options.options[index].value === role_id) {

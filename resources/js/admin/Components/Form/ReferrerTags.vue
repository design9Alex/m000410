  ...
  <div class="form-group row" :class="{'len': configs.language}">
    ...
    <div :class="['col-sm-' + options.size]">
      <b-form-tags
        ...
        :input-attrs="{list: configs.key.replaceAll('.', '-') + '-list'}"
        :placeholder="options.placeholder"
        tag-variant="info"
        separator=";"
        remove-on-delete
        :tag-validator="getSuggest"
        v-model="currentTags"></b-form-tags>
      <datalist :id="configs.key.replaceAll('.', '-') + '-list'" v-if="filled(suggestItems)">
        <option v-for="item in suggestItems" :value="item.value">{{ item.label }}</option>
      </datalist>
    </div>
...
export default {
  ...
  methods: {
    getSuggest: function (keyword) {
      this.suggestItems = [];
      if (this.filled(this.options.api_referrer)) {
        axios.get(this.options.api_referrer + '?keyword=' + keyword)
          .then((response) => {
            if (response.data.code === '0000') {
              this.suggestItems = response.data.data;
            }
          });
      }

<template>
  <admin-layout :urls="urls" :configs="configs" :trans="trans" :breadcrumbs="breadcrumbs" v-slot:default="slotProps" ref="base_edit">
    ...
    <page-inbox-reply-create
      v-if="filled(array_get(urls, 'api_reply.store')) && !originalValues.archive"
      :urls="array_get(urls, 'api_reply')"
...
import DefaultEdit from "../../Components/Mixins/DefaultEdit";
...
import PageInboxOwnerInput from "../../Components/PageInboxReceived/OwnerInput";

export default {
  name: "InboxReceivedEdit",
  mixins: [DefaultEdit],
  components: {
    AdminLayout,
    SitePageActions,
  ...
  methods: {
    ...
    switchStarred: function () {
      this.liveValues = JSON.parse(JSON.stringify(this.originalValues));
      this.liveValues.starred = !this.liveValues.starred;
      this.$nextTick(() => {
        this.submitForm(this.trans);
      });
    }
  }
}

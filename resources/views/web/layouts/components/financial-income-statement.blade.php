<div class="fnclrptUnitBox d-flex align-items-center justify-content-between" data-aos="fade-up" data-aos-duration="1000">
  ...
  <p class="fnclrptUnit mb-0">@lang('web.financial.unit_2'){{--單位：元--}}</p>
</div>

{{--data-when="year"  data-when="quarter"--}}
<div class="fnclrptChartBox jsFnclrptChart" data-when="{{$type}}" data-kind="income_statement" data-aos="fade-up" data-aos-duration="1000">
  ...
  <table>
    ...
    @if($type == 'quarter')
      @foreach($tableQuarter as $key => $item)
        ...
        <tr class="">
          <td class="title text-start fontWeight500 ">{{__('web.income_statement.'.$key)}}{{--營業收入--}}</td>
